...
8. **管线分步对话化（先确认大纲再写正文）**——现有管线是原子六步，拆步是架构级改动且与「agent 驾驶」价值重叠；登记开放问题（OD-4）。**2026-08-24 已关闭**：不拆管线，OD-4 以「启动 brief 合同」另解（意图前置于 run 之前，用户可自带标题/思路/大纲/来源），进 v0.5——见 `docs/v0.5-launch-brief.md`（ADR-010 / D-11）。
...
| OD-4 | design-decision-to-evaluate | 管线分步对话化（大纲确认 gate→成稿）——旅程中「先看大纲」是高频候选诉求 | **已关闭（2026-08-24）**：不拆管线（原子六步不动），改为「启动 brief 合同」把意图前置于 run 之前——用户可自带标题/思路/大纲/来源，分层绑定；进 v0.5，见 `docs/v0.5-launch-brief.md`（D-11 / ADR-010） |

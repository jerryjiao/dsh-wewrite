...
var GlobalStateSchema = z4.strictObject({
  ...
  agentToolsTouched: z4.boolean().default(false)
});
var INITIAL_GLOBAL = {
  v: 1,
...
function unwrapCredential(resolved) {
  if (typeof resolved === "string") return resolved.length > 0 ? resolved : void 0;
...
// src/host/agent-tools/push-approval.ts
var PUSH_TOOL_NAME = "wewrite_push_draft";
function extractArticleId(args) {
  if (typeof args === "string") return args;
  const value = args?.article_id;
  return typeof value === "string" ? value : "";
}
function safeLookupTitle(service, articleId) {
  try {
...
function describeGateVerdict(service, articleId) {
  try {
...
function composePushAskReason(service, args) {
...
function createPushApproval(ctx, service) {
  let stop;
  try {
    stop = ctx.on?.("tools/pre-execute", ((exec, next) => {
      const call = exec;
...
// src/host/agent-tools/output-helpers.ts
...
function textBlocks(...lines) {
  return lines.filter((line) => line !== "").map((text) => ({ type: "text", text }));
}
...
function asArgsRecord(args) {
  return typeof args === "object" && args !== null ? args : {};
}
function optionalString(value) {
  return typeof value === "string" && value !== "" ? value : void 0;
}
function errorToCodeMessage(error, fallbackCode) {
  const code = error instanceof Error && "code" in error && typeof error.code === "string" ? error.code : fallbackCode;
  const message = error instanceof Error ? error.message : String(error ?? "\u672A\u77E5\u9519\u8BEF");
  return { code, message };
}
function callView(kind, title, rawInput) {
  return { card: "generic", kind, title, ...rawInput ? { rawInput } : {} };
}
function resultView(title, content) {
  return { card: "generic", title, content: [...content] };
}

// src/host/agent-tools/edit-tools.ts
var REWRITE_TEXT_MAX = 8e3;
var REWRITE_INSTRUCTION_MAX = 200;
var TITLE_MAX = 200;
...
function buildRewriteTool(service) {
  return {
...
function buildPushTool(service, approval) {
  return {
    name: "wewrite_push_draft",
    description: "\u628A\u4E00\u7BC7\u5DF2\u6E32\u67D3\u7684\u6587\u7AE0\u63A8\u9001\u5230\u5FAE\u4FE1\u516C\u4F17\u53F7\u8349\u7A3F\u7BB1\uFF08\u53EA\u8FDB\u8349\u7A3F\u7BB1\uFF0C\u4E0D\u7FA4\u53D1\uFF1B\u7FA4\u53D1\u6C38\u8FDC\u7531\u53F7\u4E3B\u5728\u516C\u4F17\u5E73\u53F0\u540E\u53F0\u4EBA\u5DE5\u6267\u884C\uFF09\u3002\u8C03\u7528\u524D\u4F1A\u5F39\u51FA\u786E\u8BA4\u9762\u677F\uFF08\u542B\u6587\u7AE0\u6807\u9898\u4E0E\u95E8\u7981\u7ED3\u8BBA\uFF09\uFF0C\u672A\u7ECF\u786E\u8BA4\u4E0D\u4F1A\u53D1\u8D77\u4EFB\u4F55\u5FAE\u4FE1 API \u8C03\u7528\u3002article_id \u53EF\u5148\u7528 wewrite_list_articles \u67E5\u8BE2\u3002",
    timeoutMs: 12e4,
    parameters: {
...
// src/host/agent-tools/list-tool.ts
var LIMIT_DEFAULT = 10;
var LIMIT_MAX = 100;
function buildListTool(service) {
  return {
    ...
    async execute(args, _exec) {
      ...
      if (rawLimit === void 0 || rawLimit < 1 || rawLimit > LIMIT_MAX) {
...
// src/host/agent-tools/run-tool.ts
...
function parseBoundedStringArray(raw, field, maxItems, maxItemLen) {
...
function parseRunArgs(args) {
  ...
  const rawCount = coerceInteger(raw.image_count ?? 0) ?? (raw.image_count === void 0 ? 0 : NaN);
  ...
  const briefTitle = typeof raw.title === "string" && raw.title.trim() ? raw.title.trim() : void 0;
  if (briefTitle && [...briefTitle].length > 64) {
    return { error: toolError("brief-title-invalid", "title \u8FC7\u957F\uFF08\u226464 \u5B57\uFF0C\u5FAE\u4FE1\u6807\u9898\u4E0A\u9650\uFF09") };
  }
  const briefApproach = typeof raw.approach === "string" && raw.approach.trim() ? raw.approach.trim() : void 0;
  if (briefApproach && [...briefApproach].length > 2e3) {
...
function recordToValue(service, record, runId) {
  ...
  const article = ok && record.articleId ? lookupArticle(service, record.articleId) : void 0;
  return {
    ok,
    runId,
...
function lookupArticle(service, articleId) {
  try {
    const detail = service.getArticle(articleId);
    return detail ? { title: detail.title, digest: detail.digest } : void 0;
  } catch {
    return void 0;
  }
}
...
function buildRunTool(service) {
  return {
    name: "wewrite_run",
    description: "\u8FD0\u884C\u4E00\u6B21 WeWrite \u516C\u4F17\u53F7\u5199\u4F5C\u7BA1\u7EBF\uFF1A\u9009\u9898\u2192\u5927\u7EB2\u2192\u6210\u7A3F\u2192\u8D28\u91CF\u95E8\u7981\u2192\u6E32\u67D3\u2192\u914D\u56FE\uFF0C\u5168\u7A0B\u6570\u5206\u949F\uFF0C\u5B8C\u6210\u540E\u8FD4\u56DE\u6587\u7AE0\u6807\u8BC6\u4E0E\u6458\u8981\u3002\u628A\u7528\u6237\u5728\u5BF9\u8BDD\u91CC\u7ED9\u51FA\u7684\u6807\u9898/\u603B\u4F53\u601D\u8DEF/\u5927\u7EB2/\u53C2\u8003\u94FE\u63A5\u84B8\u998F\u8FDB title/approach/outline/sources \u5BF9\u5E94\u53C2\u6570\uFF08\u5206\u5C42\u786C\u7EA6\u675F\uFF1A\u6807\u9898\u4E0E\u601D\u8DEF\u7167\u529E\u3001\u5927\u7EB2\u8282\u540D\u539F\u6837\u4FDD\u7559\u3001\u7ED9\u5B9A\u6765\u6E90\u5FC5\u987B\u4EE5\u53EF\u89C1 URL \u5F15\u7528\u4E14\u4E0D\u5F97\u7F16\u9020\u5176\u4ED6\u6765\u6E90\uFF09\u3002\u7528\u6237\u53EA\u7ED9\u4E00\u53E5\u8BDD\u4E3B\u9898\u65F6\u4E0D\u8FFD\u95EE\u3001\u76F4\u63A5\u8FD0\u884C\u3002\u4EC5\u5728\u7528\u6237\u660E\u786E\u8868\u8FBE\u5199\u4F5C\u610F\u56FE\u65F6\u8C03\u7528\uFF1B\u8FD4\u56DE\u503C\u53EA\u8FDB\u8349\u7A3F\u76F8\u5173\u6D41\u7A0B\uFF0C\u672C\u63D2\u4EF6\u4E0D\u7FA4\u53D1\u3002",
    timeoutMs: 6e5,
    parameters: {
    ...
    presentCall: (args) => {
      ...
      if (raw.image_count !== void 0) rawInput.image_count = raw.image_count;
      for (const field of ["title", "approach", "outline", "sources"]) {
        if (raw[field] !== void 0) rawInput[field] = raw[field];
      }
      const extras = [];
      ...
      const suffix = extras.length ? `\uFF08${extras.join("\xB7")}\uFF09` : "";
...
// src/host/agent-tools/suggest-topics.ts
var COUNT_DEFAULT = 3;
var COUNT_MAX = 5;
function buildSuggestTopicsTool(service) {
  return {
    name: "wewrite_suggest_topics",
    description: "\u4ECE\u70ED\u699C\u6293\u53D6\u5019\u9009\u9009\u9898\uFF08\u9ED8\u8BA4 3 \u6761\uFF0C\u4E0A\u9650 5 \u6761\uFF09\u5E76\u9010\u6761\u751F\u6210\u4E00\u53E5\u8BDD AI \u901F\u89C8\uFF0C\u4F9B\u7528\u6237\u6311\u4E00\u4E2A\u518D\u542F\u52A8\u5199\u4F5C\u7BA1\u7EBF\u3002\u4EC5\u5728\u7528\u6237\u8868\u8FBE\u300C\u4E0D\u77E5\u9053\u5199\u4EC0\u4E48 / \u7ED9\u6211\u63A8\u8350\u9009\u9898\u300D\u7C7B\u610F\u56FE\u65F6\u8C03\u7528\uFF1B\u5448\u73B0\u5019\u9009\u540E\u7B49\u7528\u6237\u9009\u5B9A\uFF0C\u4E0D\u8981\u64C5\u81EA\u76F4\u63A5\u5F00\u8DD1\u7BA1\u7EBF\u3002",
    timeoutMs: 6e4,
    parameters: {
    ...
    output: {
      ...
      presentationMeta: (_args, value) => ({
        ...
        topics: Array.isArray(asArgsRecord(value).topics) ? asArgsRecord(value).topics.map((item) => {
          const record = asArgsRecord(item);
          return {
    ...
    async execute(args, _exec) {
      ...
      if (rawCount === void 0 || rawCount < 1 || rawCount > COUNT_MAX) {
...
async function safeDigest(service, item) {
  try {
    const record = asArgsRecord(item);
...
// src/host/agent-tools/index.ts
function warn(message) {
  console.warn(`dsh-wewrite: ${message}`);
}
function readGate(service, options) {
  const dynamic = service.agentToolsEnabled?.();
  return typeof dynamic === "boolean" ? dynamic : options.enabled;
}
function registerAgentTools(ctx, service, options) {
  const gateFace = service;
  ...
  const mount = (agent) => {
    if (!readGate(service, options)) return;
    try {
      for (const definition of definitions) {
        const stop = agent.ctx.tools.register(definition);
        if (typeof stop === "function") mountedStops.push(stop);
      }
    } catch (error) {
...
// src/host/agent-tools/commands.ts
var COMMAND_NAME = "wewrite";
function parseTopic(rawInput) {
  return rawInput.trim();
}
function registerWewriteCommand(ctx, service) {
  try {
...
var HOST_RPC_ERROR_CODE = "internal";
function toHostRpcErrorEnvelope(error, truncate) {
  const rawCode = typeof error?.code === "string" && error.code ? error.code : "rpc-failed";
...
function outlineUserPrompt(topic, brief, retryMissing) {
  const skeleton = brief?.outline?.length ? brief.outline : void 0;
  if (skeleton) {
    return [
      `\u4E3B\u9898\uFF1A${topic}`,
      "",
      ...
      ...retryMissing?.length ? ["", `\u4E0A\u4E00\u6B21\u8F93\u51FA\u9057\u6F0F\u4E86\u4EE5\u4E0B\u7ED9\u5B9A\u8282\uFF0C\u672C\u6B21\u5FC5\u987B\u539F\u6837\u5305\u542B\uFF1A${retryMissing.map((section) => `\u300C${section}\u300D`).join("\u3001")}`] : []
    ].join("\n");
  }
  return [
    `\u4E3B\u9898\uFF1A${topic}`,
...
function draftUserPrompt(topic, outline, brief, retryMissingOutline, retryInvisibleSources) {
  ...
  if (retryMissingOutline?.length) {
    lines.push(
      "",
      `\u4E0A\u4E00\u6B21\u6210\u7A3F\u9057\u6F0F/\u6539\u5199\u4E86\u4EE5\u4E0B\u7ED9\u5B9A\u8282\u540D\uFF0C\u672C\u6B21\u5FC5\u987B\u539F\u6837\u4FDD\u7559\uFF08\u8282\u540D\u4E00\u5B57\u4E0D\u6539\uFF09\uFF1A${retryMissingOutline.map((section) => `\u300C${section}\u300D`).join("\u3001")}`
    );
  }
...
function rewriteUserPrompt(input) {
  return [
...
function matchBlock(html, tag) {
  const match = new RegExp(`<${tag}\\b[^>]*>([\\s\\S]*?)<\\/${tag}\\s*>`, "i").exec(html);
  return match ? match[1] : null;
}
var BROWSER_REQUEST_HEADERS = {
...
async function fetchArticleText(url, fetchImpl, timeoutMs = FETCH_TIMEOUT_MS) {
...
function digestItemSystemPrompt() {
  return [
...
function digestItemArticleUserPrompt(title, domain, body) {
  return [
    `\u70ED\u699C\u6761\u76EE\u6807\u9898\uFF1A${title}`,
    `\u6765\u6E90\u57DF\u540D\uFF1A${domain}`,
    "",
    ...
    "\u8FD9\u6761\u5728\u8BB2\u4EC0\u4E48\uFF1A\u4E00\u53E5\u8BDD\u6982\u62EC\u6838\u5FC3\u4E8B\u4EF6",
    "\xB7 \u8981\u70B9\uFF1A\u5177\u4F53\u4E8B\u5B9E\u6216\u6570\u5B57\uFF082 \u5230 4 \u884C\uFF0C\u6BCF\u884C\u4E00\u4E2A\uFF09",
    "\u8981\u70B9\u53EA\u80FD\u6765\u81EA\u7ED9\u5B9A\u6B63\u6587\uFF0C\u4E0D\u5F97\u8865\u5145\u5916\u90E8\u4FE1\u606F\uFF0C\u4E0D\u5F97\u865A\u6784\u6570\u5B57\u4E0E\u4E8B\u5B9E\u3002"
  ].join("\n");
}
function digestItemTitleUserPrompt(title, domain) {
  return [
    `\u70ED\u699C\u6761\u76EE\u6807\u9898\uFF1A${title}`,
    `\u6765\u6E90\u57DF\u540D\uFF1A${domain}`,
    "",
    ...
    "\xB7 \u89D2\u5EA6\uFF1A\u4ECE\u516C\u4F17\u53F7\u9009\u9898\u89C6\u89D2\u7ED9\u4E00\u4E2A\u53EF\u5199\u7684\u89D2\u5EA6\uFF0C\u4E00\u884C",
    "\u53EA\u6709\u6807\u9898\u4E0E\u57DF\u540D\u53EF\u7528\uFF1A\u4E0D\u5F97\u865A\u6784\u539F\u6587\u6CA1\u6709\u7684\u4E8B\u5B9E\uFF1B\u89D2\u5EA6\u53EA\u505A\u65B9\u5411\u6027\u5EFA\u8BAE\uFF0C\u4E0D\u63D0\u5177\u4F53\u529F\u80FD\u3001\u6570\u5B57\u6216\u53C2\u6570\u3002"
  ].join("\n");
}
async function digestHotspotItem(deps, item) {
  ...
  try {
    const outcome = await streamLlmText(
      deps.llm,
      {
        ...
        user: articleText !== null ? digestItemArticleUserPrompt(item.title, domain, articleText) : digestItemTitleUserPrompt(item.title, domain),
        provider,
        model,
    ...
    if (outcome.status === "aborted") {
      throw new WewriteServiceError("digest-timeout", `AI \u901F\u89C8\u751F\u6210\u8D85\u65F6\uFF08${Math.round(deps.timeoutMs / 1e3)} \u79D2\uFF09\uFF0C\u5DF2\u53D6\u6D88\uFF0C\u8BF7\u91CD\u8BD5`);
    }
...
function visibleContainsUrl(visibleText, url) {
  const trimmed = url.trim();
  const variants = [trimmed, trimmed.replace(/\/+$/, "")];
  return variants.some((variant) => variant.length > 0 && visibleText.includes(variant));
}
function auditSourceVisibility(markdown, sources) {
  const visibleText = stripLinkUrls(markdown);
  const issues = sources.filter((url) => !visibleContainsUrl(visibleText, url)).map((url) => `\u6765\u6E90\u672A\u4EE5\u53EF\u89C1 URL \u51FA\u73B0\u5728\u6B63\u6587\uFF08\u5FAE\u4FE1\u4F1A\u5265\u79BB\u94FE\u63A5\u8BED\u6CD5\uFF0C\u9700\u5199\u6210\u88F8\u6587\u672C\uFF09\uFF1A${url}`);
  return { passed: issues.length === 0, issues };
}
function isSourceUrlVisible(markdown, url) {
  return visibleContainsUrl(stripLinkUrls(markdown), url);
}
function auditInventedSources(markdown, allowed) {
  const allowedKeys = allowed.map(urlKey).filter(Boolean);
  const allowedByPrefix = (key) => allowedKeys.some(
...
function runQualityGates(input) {
  ...
  const sources = input.sources?.length ? (() => {
    const userUrls = (input.userText ?? []).flatMap(
      (text2) => [...text2.matchAll(URL_PATTERN)].map((match) => match[0].replace(/[.,;:，。；：、]+$/, ""))
    );
    ...
    const issues2 = [...visibility.issues, ...invented.issues];
    return { passed: issues2.length === 0, issues: issues2 };
  })() : void 0;
  const outlineSkeleton = input.outlineSkeleton?.length ? (() => {
    const issues2 = input.outlineSkeleton.filter((section) => !text.includes(section)).map((section) => `\u5927\u7EB2\u9AA8\u67B6\uFF1A\u7ED9\u5B9A\u8282\u300C${section}\u300D\u672A\u539F\u6837\u51FA\u73B0\u5728\u6210\u7A3F`);
    return { passed: issues2.length === 0, issues: issues2 };
  })() : void 0;
  const issues = [];
...
var TERMINAL_STATUSES2 = /* @__PURE__ */ new Set(["succeeded", "failed", "cancelled", "interrupted"]);
function pruneTerminalRuns(runs, limit) {
  const active = runs.filter((run) => !TERMINAL_STATUSES2.has(run.status));
  const terminal = runs.filter((run) => TERMINAL_STATUSES2.has(run.status)).sort((left, right) => Date.parse(right.finishedAt ?? right.startedAt) - Date.parse(left.finishedAt ?? left.startedAt));
...
function createPipelineEngine(deps) {
  ...
  const completionWaiters = /* @__PURE__ */ new Map();
    ...
    for (const stepName of PIPELINE_STEP_NAMES) {
      ...
      try {
        if (stepName === "topic") {
          ...
          const skeleton = params.brief?.outline?.length ? params.brief.outline : void 0;
          let retryMissing;
          let outlineText = "";
          ...
          const llmCall = resolveLlmCall(params);
          const skeleton = params.brief?.outline?.length ? params.brief.outline : void 0;
          const briefSources = params.brief?.sources?.length ? params.brief.sources : void 0;
          let retryMissing;
          let retryInvisible;
          let draftText = "";
          for (let attempt = 0; ; attempt += 1) {
            ...
            const invisible = briefSources ? briefSources.filter((url) => !isSourceUrlVisible(draftText, url)) : [];
            if (!missing.length && !invisible.length || attempt >= 1) break;
            retryMissing = missing.length ? missing : void 0;
            retryInvisible = invisible.length ? invisible : void 0;
          }
          draft = draftText;
          ...
          if (!verdict.passed) {
            const issues = verdict.report.issues;
            const summary = Array.isArray(issues) && issues.length ? `\u8D28\u91CF\u95E8\u7981\u672A\u901A\u8FC7\uFF1A${issues.slice(0, 3).map(String).join("\uFF1B")}${issues.length > 3 ? " \u7B49" : ""}` : "\u8D28\u91CF\u95E8\u7981\u672A\u901A\u8FC7\uFF0C\u9ED8\u8BA4\u63A8\u9001\u8DEF\u5F84\u5DF2\u88AB\u963B\u65AD";
  ...
  function awaitDone(runId) {
    const current = deps.store.get(runId);
    if (!current || TERMINAL_STATUSES2.has(current.status)) return Promise.resolve(current);
    const waiters = completionWaiters.get(runId) ?? /* @__PURE__ */ new Set();
...
// src/host/agent-tools-gate.ts
function createAgentToolsGate(source) {
  const listeners = /* @__PURE__ */ new Set();
  return {
    enabled: () => source.touched() ? source.explicit() : source.configDefault,
    subscribe(listener) {
...
// src/host/call-run-bindings.ts
var CALL_RUN_BINDINGS_LIMIT = 500;
function createCallRunBindings(limit = CALL_RUN_BINDINGS_LIMIT) {
...
// src/host/service.ts
var HOTSPOT_DIGEST_TIMEOUT_MS = 45e3;
var REWRITE_TIMEOUT_MS = 45e3;
var WeWriteService = class _WeWriteService {
  ...
  async digestHotspotItem(item) {
    return digestHotspotItem(
      {
        llm: this.deps.llm,
    ...
    try {
      const outcome = await streamLlmText(
        this.deps.llm,
        {
          ...
          provider,
          model,
          maxTokens: Math.min(4e3, input.text.length * 3 + 500)
        },
        controller.signal
      );
      if (outcome.status === "aborted") {
        throw new WewriteServiceError("rewrite-timeout", `AI \u6539\u5199\u8D85\u65F6\uFF08${Math.round(timeoutMs / 1e3)} \u79D2\uFF09\uFF0C\u5DF2\u53D6\u6D88\uFF0C\u8BF7\u91CD\u8BD5`);
      }
      if (outcome.status === "error") throw new WewriteServiceError(outcome.code, outcome.message);
  ...
  runDetail(selector) {
    const runId = selector.runId ?? (selector.callId ? this.callBindings.resolve(selector.callId) : void 0);
    const record = runId ? this.runStore.get(runId) : void 0;
  ...
  bindRunCall(callId, runId) {
    this.callBindings.bind(callId, runId);
  }
  lookupArticleTitle(articleId) {
    const record = articleId ? this.tables.articles.get(articleId) : void 0;
  ...
  agentToolsEnabled() {
    return this.agentToolsGate.enabled();
  }
  onAgentToolsChanged(listener) {
    return this.agentToolsGate.subscribe(listener);
  }
  async getConfig() {
  ...
  async setConfig(patch) {
    return this.serialize(async () => {
      ...
      this.state = { ...this.state, settings: parsed.data, ...Object.prototype.hasOwnProperty.call(patch, "agentToolsEnabled") ? { agentToolsTouched: true } : {} };
      await this.persistState();
...
var inject = ["storageDomain", "agents", "sessions", "connection", "llm", "credentials", "tools", "commands"];

/**
 * Agent 工具面 E2 契约（architecture §2.3）：canonical value 与 presentationMeta 的 zod schema。
 * 硬约束：meta 随 tool/result 事件持久化，Session.append 对 meta 跑 isJsonValue 运行时校验，
 * 非 JSON 在写入点抛错——所以每个 meta 必须 strict 拒未知字段（契约漂移防护）且
 * JSON.stringify→parse 无损 round-trip（勘误 1：E2 是终局投影的唯一持久化载体）。
 * tool 字面量是 E3 deliverables 的识别标记，错值/缺失一律拒。
 */
import { z } from 'zod';
/** wewrite_run 终态枚举（execute 只在终态 settle；running/queued 等非终态拒）。 */
export declare const RUN_TOOL_TERMINAL_STATUSES: readonly ["succeeded", "failed", "cancelled", "interrupted"];
/** wewrite_run 的 canonical value（execute 返回，过 output.schema 校验）。 */
export declare const RunToolValueSchema: z.ZodObject<{
    ok: z.ZodBoolean;
    runId: z.ZodString;
    status: z.ZodEnum<{
        failed: "failed";
        succeeded: "succeeded";
        cancelled: "cancelled";
        interrupted: "interrupted";
    }>;
    articleId: z.ZodOptional<z.ZodString>;
    title: z.ZodOptional<z.ZodString>;
    digest: z.ZodOptional<z.ZodString>;
    gatePassed: z.ZodOptional<z.ZodBoolean>;
    error: z.ZodOptional<z.ZodObject<{
        code: z.ZodString;
        message: z.ZodString;
    }, z.core.$strict>>;
}, z.core.$strict>;
export type RunToolValue = z.infer<typeof RunToolValueSchema>;
/** E2 meta：value 超集 + E3 识别标记 + presentCall 侧入参快照。 */
export declare const RunToolMetaSchema: z.ZodObject<{
    ok: z.ZodBoolean;
    runId: z.ZodString;
    status: z.ZodEnum<{
        failed: "failed";
        succeeded: "succeeded";
        cancelled: "cancelled";
        interrupted: "interrupted";
    }>;
    articleId: z.ZodOptional<z.ZodString>;
    title: z.ZodOptional<z.ZodString>;
    digest: z.ZodOptional<z.ZodString>;
    gatePassed: z.ZodOptional<z.ZodBoolean>;
    error: z.ZodOptional<z.ZodObject<{
        code: z.ZodString;
        message: z.ZodString;
    }, z.core.$strict>>;
    tool: z.ZodLiteral<"wewrite_run">;
    topic: z.ZodString;
}, z.core.$strict>;
export type RunToolMeta = z.infer<typeof RunToolMetaSchema>;
export declare const PushToolMetaSchema: z.ZodObject<{
    tool: z.ZodLiteral<"wewrite_push_draft">;
    articleId: z.ZodString;
    title: z.ZodString;
    ok: z.ZodBoolean;
    mediaId: z.ZodOptional<z.ZodString>;
    error: z.ZodOptional<z.ZodObject<{
        code: z.ZodString;
        message: z.ZodString;
    }, z.core.$strict>>;
}, z.core.$strict>;
export type PushToolMeta = z.infer<typeof PushToolMetaSchema>;
export declare const RewriteToolMetaSchema: z.ZodObject<{
    tool: z.ZodLiteral<"wewrite_rewrite">;
    charsIn: z.ZodNumber;
    charsOut: z.ZodNumber;
    ok: z.ZodBoolean;
    error: z.ZodOptional<z.ZodObject<{
        code: z.ZodString;
        message: z.ZodString;
    }, z.core.$strict>>;
}, z.core.$strict>;
export type RewriteToolMeta = z.infer<typeof RewriteToolMetaSchema>;
/** 热榜选题候选条目（Spec §5 第 5 工具）：来源 + 标题 + AI 速览，多余键拒（不泄漏 url）。 */
export declare const SuggestTopicItemSchema: z.ZodObject<{
    title: z.ZodString;
    source: z.ZodString;
    digest: z.ZodString;
}, z.core.$strict>;
export type SuggestTopicItem = z.infer<typeof SuggestTopicItemSchema>;
export declare const SuggestTopicsMetaSchema: z.ZodObject<{
    tool: z.ZodLiteral<"wewrite_suggest_topics">;
    topics: z.ZodArray<z.ZodObject<{
        title: z.ZodString;
        source: z.ZodString;
        digest: z.ZodString;
    }, z.core.$strict>>;
}, z.core.$strict>;
export type SuggestTopicsMeta = z.infer<typeof SuggestTopicsMetaSchema>;
